...
async function fetchCount(protocol: string, startdate: string, enddate: string): Promise<number> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`GLOBE API error (${response.status}): ${message}`);
  }
...
export const GET: APIRoute = async ({ request }) => {
  ...
  const protocols = protocolsParam
    ...
    : ['clouds', 'mosquito_habitat_mapper', 'land_covers', 'tree_heights'];
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ error: error instanceof Error ? error.message : 'Failed to load stats' }),
